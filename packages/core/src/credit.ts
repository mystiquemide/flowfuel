...
/**
 * Decodes an `Activation` event log. The event is anonymous-free:
 * topic[0] is the event signature, topic[1] the activation ID, topic[2] the
 * activated account, topic[3] the payer, and data carries the amount.
 */
...
export function decodeActivationLog(log: {
  ...
  data: string;
}): { activationId: number; account: string; payer: string; amount: bigint } | null {
  ...
  const payer = `0x${log.topics[3]!.slice(-40)}`;
  const amount = BigInt(log.data);
  return { activationId, account, payer, amount };
}
